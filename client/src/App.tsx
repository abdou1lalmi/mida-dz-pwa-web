 ...
 * Warm editorial food imagery, deep olive utility, saffron-only active signals, multilingual care.
 */
...
function HomePage({ t, language, saved, onOpen, onToggleSave, onDiscover, onPickRegion }: { t: UiCopy; language: Language; saved: string[]; onOpen: (recipe: Recipe) => void; onToggleSave: (id: string) => void; onDiscover: () => void; onPickRegion: (region: string) => void }) {
  ...
  return <>
    <section className="page-head page-head--home"><p className="eyebrow">{t.date}</p><h1>{t.homeLead}<br /><i>{t.homeAccent}</i></h1><button className="quiet-action" onClick={onDiscover}>{t.explore} <ArrowLeft size={16} /></button></section>
